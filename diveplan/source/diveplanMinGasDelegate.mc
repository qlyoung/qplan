...
class diveplanMinGasDelegate extends WatchUi.Menu2InputDelegate {
    private var _settings as MinGasSettings;
    private var _menu as WatchUi.Menu2;

    function initialize(menu as WatchUi.Menu2) {
        Menu2InputDelegate.initialize();
        _settings = new MinGasSettings();
        _menu = menu;
        updateMenuLabels();
    }
    ...
    function onSelect(item) as Void {
        ...
        if (item.getId() == :bottom_depth) {
            // TODO: Create depth selection for bottom depth
        } else if (item.getId() == :next_gas_depth) {
